...
import app.gaborbiro.flathunt.TravelLimit
import app.gaborbiro.flathunt.TravelMode
...
class DirectionsServiceImpl : DirectionsService, KoinComponent {
    ...
    override fun calculateRoutes(from: DirectionsLatLon, targets: Collection<POI>): List<Route> {
        return targets.mapNotNull { target ->
            calculateRoutes(from, target)
        }
    }

    private fun calculateRoutes(from: DirectionsLatLon, target: POI): Route? {
        return when (target) {
            is POI.Destination -> {
                getDirectionsTo(
                    from = from,
                    to = DirectionsLatLon(target.coordinates.latitude, target.coordinates.longitude),
                    travelLimits = target.max,
                    ...
                    Route(
                        ...
                        name = target.name,
                        coordinates = DirectionsLatLon(target.coordinates.latitude, target.coordinates.longitude),
                    )
                }
            }

            is POI.NearestRailStation -> {
                getRoutesToNearestStations(from = from).minByOrNull { it.timeMinutes }
            }
        }
    }
...
//    }
    ...
    override fun getRoutesToNearestStations(from: DirectionsLatLon): List<Route> {
