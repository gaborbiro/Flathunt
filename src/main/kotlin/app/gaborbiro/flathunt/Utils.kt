...
fun command(
    command: String, description: String, exec: (Unit) -> Unit
) = object : Command<Unit>(command, description, emptyList(), exec) {}
...
) = object : Command<Single<A>>(command, description, listOf(argumentName), exec) {}
...
) = object : Command<Pair<A, B>>(command, description, listOf(argumentName1, argumentName2), exec) {}
...
inline fun <reified A, reified B, reified C> command(
    ...
    Command<Triple<A, B, C>>(command, description, listOf(argumentName1, argumentName2, argumentName3), exec) {}
