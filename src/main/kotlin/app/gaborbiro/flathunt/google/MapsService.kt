...
fun calculateRoutes(property: Property, pois: Collection<POI>): List<Route> {
    return property.location?.let { location: LatLon ->
        pois.mapNotNull { poi ->
            when (poi) {
                is POI.Explicit -> {
                    getDirectionsTo(
                ...
                is POI.NearestRailStation -> {
                    getRoutesToNearestStations(from = location).minByOrNull { it.timeMinutes }
                }
            }
        }
    } ?: emptyList()
                    ...
                    val collapsedSteps = leg.steps.fold(mutableListOf<RouteStep>()) { steps, step ->
                        if (steps.isEmpty()) {
                            ...
                            if (last.travelMode == TravelMode.CYCLING.value.uppercase()) {
                                if (step.repleacable()) { // fold it in
...
private fun RouteStep.repleacable() = travelMode == TravelMode.WALKING.value.uppercase() ||
...
fun getRoutesToNearestStations(from: LatLon): List<Route> {
