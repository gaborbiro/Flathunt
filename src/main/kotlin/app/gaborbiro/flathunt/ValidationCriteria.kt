...
fun Property.validate(criteria: ValidationCriteria): List<String> {
    ...
    if (this.furnished != null) {
        if (criteria.furnished == true && !this.furnished) {
            errors.add("unfurnished")
        }
        if (criteria.furnished == false && this.furnished) {
            errors.add("furnished")
        }
    }
    if (this.livingRoom != null) {
        if (criteria.sharedLivingRoom == true && !this.livingRoom) {
            errors.add("no living room")
        }
        if (criteria.sharedLivingRoom == false && this.livingRoom) {
            errors.add("has living room")
        }
    }
    if (criteria.maxFlatmates != null && flatmates != null && this.flatmates > criteria.maxFlatmates) {
        errors.add("more than ${criteria.maxFlatmates} flatmates")
    }
    if (criteria.maxBedrooms != null && this.totalRooms != null && this.totalRooms > criteria.maxBedrooms) {
        errors.add("more than ${criteria.maxBedrooms} bedrooms")
    }
    if (criteria.minBedrooms != null && this.totalRooms != null && this.totalRooms < criteria.minBedrooms) {
